...
load("brushing.fingerprint15s.RDA")
load("brushing.fingerprint.sd15s.RDA")
brushing.fingerprint <- as.data.frame(brushing.fingerprint)
brushing.fingerprint.sd <- as.data.frame(brushing.fingerprint.sd)
...
format.df <- function(df) {
  ...
  df <- df %>% mutate(vector.mag = sqrt(AccelerometerX^2+ AccelerometerY^2+ AccelerometerZ^2),
                      time_minute = as.POSIXct(floor(as.double(Timestamp)/(15))*(15),origin=(as.POSIXlt('1970-01-01'))))  
  return(df)
}
...
summary.df <- function(df, freq, k=10){
  ...
  test.summary <- select(test, -vector.dir, -time_minute) %>%
    ...
    mutate(Timestamp = as.POSIXct(floor(as.double(Timestamp)/(15))*(15),origin=(as.POSIXlt('1970-01-01')))) %>%
